...
HKUST(GZ)   &  21+499          & \ \cmark   & \cmark   & \cmark   & \ \ \quad \cmark   & Phase/structure identification dataset  \\ 
USC         &  338          & \ \cmark   & \cmark   & \partialcheck{90}   & \ \ \quad \xmark   & Study of CuNi and CuAl alloys          \\ 
EMPA        &  790          &\ \cmark   & \partialcheck{63}   & \xmark   & \ \ \quad \xmark   & Tin halide perovskites, Zn-V-N libraries \\ 
INT         &  19,780        &\ \xmark   & \xmark   & \xmark   & \ \ \quad \xmark   & Compilation of various projects        \\ 
IKFT        &  64           &\ \xmark   & \xmark   & \xmark   & \ \ \quad \xmark   & Commercial catalysts, bulk materials   \\ 
LBNL        &  n/a          &\ \xmark   & \xmark   & \xmark   & \ \ \quad \xmark   & Perovskites precursors, Mn-Sb-O system  \\ 
\bottomrule
\end{tabular}
\end{adjustbox}}
\end{table}

The below figure~$\eqref{fig:components}$ illustrates how varied the data of each contribution is using principle component analysis (PCA). PCA is a dimensional reduction technique that can be applied to datasets $X \subset \mathbb{R}^n$ to reduce the number of components needed to describe points $\p \in X$ up to some tolerance in lost accuracy. The cumulative explained variance ratio is a measure indicating how much of the variance in the dataset $X$ can be explained using a given number of components. For example, if PCA on a dataset $X \subset \mathbb{R}^10$ yields a cumulative variance ratio of 99\%, then each point in the dataset can be represented using only 5 degrees of freedom up to a minor loss in accuracy.
